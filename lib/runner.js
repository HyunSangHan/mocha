...
Runner.prototype.hook = function(name, fn) {
  ...
  function next(i) {
    ...
    hook.allowUnhandled = self.allowUnhandled;
...
Runner.prototype.runTest = function(fn) {
  ...
  if (this.allowUncaught) {
    ...
    return test.run(fn);
  } else if (this.allowUnhandled) {
    test.allowUnhandled = true;
    return test.run(fn);
  }
  try {
    test.run(fn);
...
/**
 * Handle uncaught exceptions and unhandled rejections within runner.
 *
 * @param {Error} err
 * @param {boolean|undefined} isUnhandled - true for unhandled rejections, false for uncaught exceptions.
 * @private
 */
Runner.prototype.uncaught = function(err, isUnhandled) {
  var type = isUnhandled ? 'unhandled' : 'uncaught';
  if (err instanceof Pending) {
    return;
  }
  ...
  if (err) {
    debug('%s exception %O', type, err);
  } else {
    debug('%s undefined/falsy exception', type);
  ...
  if (!runnable) {
    var capitalizedType = isUnhandled ? 'Unhandled' : 'Uncaught';

    runnable = new Runnable(capitalizedType + ' error outside test suite');
...
Runner.prototype.run = function(fn) {
  ...
  function uncaught(err) {
    self.uncaught(err, false);
  }

  function unhandled(err) {
    self.uncaught(err, true);
  }
